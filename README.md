...
| **Stamps: the one exception** | **A non-note moved by an agent carries no stamp: a PNG has nowhere to put YAML, and a `.canvas` or `.base` is parsed whole by Obsidian, so frontmatter prepended to one is a broken document rather than an annotated one.** This is a genuine hole in the rule above, not an oversight, and it is stated here rather than left for someone to discover from a dataview query that misses files. What records such a move instead: the snapshot commit, and the `move_file` line in `vault-mcp`'s audit log. If the vault runs `EXCLUDE_ATTACHMENTS=1` the commit is not one of them either for media — `snapshot.sh` excludes binary types only, so a `.canvas` or `.base` move is still in git. A sidecar `.md` per attachment was considered and rejected — it doubles the file count in the vault to describe files Obsidian already shows you, and Sync would carry the sidecar and the image as unrelated objects. |
| **Deleting** | **Nothing may destroy a note or an attachment.** That is the rule the delete tools added on 2026-09-05 are shaped to keep: `trash_file` removes a `.base` or a `.canvas` by renaming it into `<vault>/.trash`, the way Obsidian's own delete does, so no byte is destroyed and it is recoverable in the app and in the snapshot history; `delete_empty_folder` removes a folder holding nothing at all, the one delete that cannot destroy content. The deletable set is a separate, smaller allow list than the movable one (`trashableExts` in `vault-mcp/vault.go`), a `.md`, an image and a PDF are refused on every surface, and the trash destination is computed from the source rather than supplied by the caller, so the dotted-path deny list is never relaxed. [Why](obsidian-vault/DECISIONS.md#deleting). |
