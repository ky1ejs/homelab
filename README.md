...
| **Non-markdown files: the 2026-08-31 exception** | `fetch_attachment` **creates** them, and it is confined to `/scratch` — never the vault. It exists because nothing else can put an image on disk at all: WebFetch returns text and never a file, `Write` cannot produce binary, `Read` cannot open a PNG, and `Bash` is denied everywhere here. The rule above is unchanged for the vault, and *reading* an attachment is still refused on every surface. The fetchable set is deliberately narrower than the movable one — no SVG, no `.canvas`, nothing whose type cannot be sniffed — because this is the one tool bringing bytes in from outside. `vault-mcp/fetch.go`. |
